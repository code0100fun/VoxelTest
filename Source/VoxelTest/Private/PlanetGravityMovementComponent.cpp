...
UPlanetGravityMovementComponent::UPlanetGravityMovementComponent(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	UE_LOG(LogTemp, Warning, TEXT("UPlanetGravityMovementComponent ctor"));
...
bool UPlanetGravityMovementComponent::DoJump(bool bReplayingMoves)
{
...
FVector UPlanetGravityMovementComponent::GetImpartedMovementBaseVelocity() const
{
...
void UPlanetGravityMovementComponent::JumpOff(AActor* MovementBaseActor)
{
...
FVector UPlanetGravityMovementComponent::GetBestDirectionOffActor(AActor* BaseActor) const
{
...
void UPlanetGravityMovementComponent::SetDefaultMovementMode()
{
...
void UPlanetGravityMovementComponent::OnMovementModeChanged(EMovementMode PreviousMovementMode, uint8 PreviousCustomMode)
{
	if (!HasValidData())
	{
		return;
...
void UPlanetGravityMovementComponent::PerformAirControlForPathFollowing(FVector Direction, float ZDiff)
{
...
void UPlanetGravityMovementComponent::PerformAirControlForPathFollowingEx(const FVector& MoveVelocity, const FVector& GravDir)
{
...
FVector UPlanetGravityMovementComponent::ConstrainAnimRootMotionVelocity(const FVector& RootMotionVelocity, const FVector& CurrentVelocity) const
{
...
void UPlanetGravityMovementComponent::SimulateMovement(float DeltaSeconds)
{
...
void UPlanetGravityMovementComponent::MaybeUpdateBasedMovement(float DeltaSeconds)
{
...
void UPlanetGravityMovementComponent::UpdateBasedMovement(float DeltaSeconds)
{
	if (!HasValidData())
	{
		return;
...
void UPlanetGravityMovementComponent::UpdateBasedRotation(FRotator& FinalRotation, const FRotator& ReducedRotation)
{
...
void UPlanetGravityMovementComponent::Crouch(bool bClientSimulation)
{
	if (!HasValidData())
	{
		return;
...
void UPlanetGravityMovementComponent::UnCrouch(bool bClientSimulation)
{
	if (!HasValidData())
	{
		return;
...
float UPlanetGravityMovementComponent::SlideAlongSurface(const FVector& Delta, float Time, const FVector& InNormal, FHitResult& Hit, bool bHandleImpact)
{
	if (!Hit.bBlockingHit)
	{
		return 0.0f;
...
void UPlanetGravityMovementComponent::TwoWallAdjust(FVector& Delta, const FHitResult& Hit, const FVector& OldHitNormal) const
{
...
FVector UPlanetGravityMovementComponent::HandleSlopeBoosting(const FVector& SlideResult, const FVector& Delta, const float Time, const FVector& Normal, const FHitResult& Hit) const
{
	const FVector CapsuleUp = GetComponentAxisZ();
...
float UPlanetGravityMovementComponent::ImmersionDepth() const
{
	float Depth = 0.0f;
...
void UPlanetGravityMovementComponent::RequestDirectMove(const FVector& MoveVelocity, bool bForceMaxSpeed)
{
...
float UPlanetGravityMovementComponent::GetMaxJumpHeight() const
{
...
void UPlanetGravityMovementComponent::PhysFlying(float deltaTime, int32 Iterations)
{
...
void UPlanetGravityMovementComponent::ApplyRootMotionToVelocityOVERRIDEN(float deltaTime)
{
...
void UPlanetGravityMovementComponent::PhysSwimming(float deltaTime, int32 Iterations)
{
	if (deltaTime < MIN_TICK_TIME)
	{
		return;
...
void UPlanetGravityMovementComponent::StartSwimmingOVERRIDEN(FVector OldLocation, FVector OldVelocity, float timeTick, float remainingTime, int32 Iterations)
{
...
FVector UPlanetGravityMovementComponent::GetFallingLateralAcceleration(float DeltaTime)
{
	return GetFallingLateralAccelerationEx(DeltaTime, GetGravityDirection(true));
}

FVector UPlanetGravityMovementComponent::GetFallingLateralAccelerationEx(float DeltaTime, const FVector& GravDir) const
{
...
FVector UPlanetGravityMovementComponent::GetAirControl(float DeltaTime, float TickAirControl, const FVector& FallAcceleration)
{
	return GetAirControlEx(DeltaTime, TickAirControl, FallAcceleration, GetGravityDirection(true));
}

FVector UPlanetGravityMovementComponent::GetAirControlEx(float DeltaTime, float TickAirControl, const FVector& FallAcceleration, const FVector& GravDir) const
{
	// Boost.
...
float UPlanetGravityMovementComponent::BoostAirControl(float DeltaTime, float TickAirControl, const FVector& FallAcceleration)
{
	return BoostAirControlEx(DeltaTime, TickAirControl, FallAcceleration, GetGravityDirection(true));
}

float UPlanetGravityMovementComponent::BoostAirControlEx(float DeltaTime, float TickAirControl, const FVector& FallAcceleration, const FVector& GravDir) const
{
...
void UPlanetGravityMovementComponent::PhysFalling(float deltaTime, int32 Iterations)
{
...
FVector UPlanetGravityMovementComponent::LimitAirControl(float DeltaTime, const FVector& FallAcceleration, const FHitResult& HitResult, bool bCheckForValidLandingSpot)
{
	return LimitAirControlEx(DeltaTime, FallAcceleration, HitResult, GetGravityDirection(true), bCheckForValidLandingSpot);
}

FVector UPlanetGravityMovementComponent::LimitAirControlEx(float DeltaTime, const FVector& FallAcceleration, const FHitResult& HitResult, const FVector& GravDir, bool bCheckForValidLandingSpot) const
{
...
bool UPlanetGravityMovementComponent::CheckLedgeDirection(const FVector& OldLocation, const FVector& SideStep, const FVector& GravDir) const
{
...
FVector UPlanetGravityMovementComponent::GetLedgeMove(const FVector& OldLocation, const FVector& Delta, const FVector& GravDir) const
{
...
void UPlanetGravityMovementComponent::StartFalling(int32 Iterations, float remainingTime, float timeTick, const FVector& Delta, const FVector& subLoc)
{
...
FVector UPlanetGravityMovementComponent::ComputeGroundMovementDelta(const FVector& Delta, const FHitResult& RampHit, const bool bHitFromLineTrace) const
{
	const FVector CapsuleUp = GetComponentAxisZ();
	return ComputeGroundMovementDeltaEx(FVector::VectorPlaneProject(Delta, CapsuleUp), CapsuleUp, RampHit, bHitFromLineTrace);
}

FVector UPlanetGravityMovementComponent::ComputeGroundMovementDeltaEx(const FVector& Delta, const FVector& DeltaPlaneNormal, const FHitResult& RampHit, const bool bHitFromLineTrace) const
{
...
void UPlanetGravityMovementComponent::MoveAlongFloor(const FVector& InVelocity, float DeltaSeconds, FStepDownResult* OutStepDownResult)
{
	if (!CurrentFloor.IsWalkableFloor())
	{
		return;
...
void UPlanetGravityMovementComponent::MaintainHorizontalGroundVelocity()
{
	if (bMaintainHorizontalGroundVelocity)
	{
...
void UPlanetGravityMovementComponent::PhysWalking(float deltaTime, int32 Iterations)
{
...
void UPlanetGravityMovementComponent::AdjustFloorHeight()
{
...
void UPlanetGravityMovementComponent::SetPostLandedPhysics(const FHitResult& Hit)
{
	if (CharacterOwner)
	{
		if (CanEverSwim() && IsInWater())
...
void UPlanetGravityMovementComponent::OnTeleported()
{
	if (!HasValidData())
	{
		return;
...
void UPlanetGravityMovementComponent::PhysicsRotation(float DeltaTime)
{
...
void UPlanetGravityMovementComponent::PhysicsVolumeChanged(class APhysicsVolume* NewVolume)
{
	if (!HasValidData())
	{
		return;
...
bool UPlanetGravityMovementComponent::ShouldJumpOutOfWater(FVector& JumpDir)
{
	return ShouldJumpOutOfWaterEx(JumpDir, GetGravityDirection(true));
}

bool UPlanetGravityMovementComponent::ShouldJumpOutOfWaterEx(FVector& JumpDir, const FVector& GravDir)
{
...
bool UPlanetGravityMovementComponent::CheckWaterJump(FVector CheckPoint, FVector& WallNormal)
{
	return CheckWaterJumpEx(CheckPoint, GetGravityDirection(true), WallNormal);
}

bool UPlanetGravityMovementComponent::CheckWaterJumpEx(FVector CheckPoint, const FVector& GravDir, FVector& WallNormal)
{
	if (!HasValidData())
	{
		return false;
...
void UPlanetGravityMovementComponent::MoveSmooth(const FVector& InVelocity, const float DeltaSeconds, FStepDownResult* OutStepDownResult)
{
	if (!HasValidData())
	{
		return;
...
bool UPlanetGravityMovementComponent::IsWalkable(const FHitResult& Hit) const
{
	if (!Hit.IsValidBlockingHit())
	{
...
bool UPlanetGravityMovementComponent::IsWithinEdgeTolerance(const FVector& CapsuleLocation, const FVector& TestImpactPoint, const float CapsuleRadius) const
{
	return IsWithinEdgeToleranceEx(CapsuleLocation, GetComponentAxisZ() * -1.0f, CapsuleRadius, TestImpactPoint);
}

bool UPlanetGravityMovementComponent::IsWithinEdgeToleranceEx(const FVector& CapsuleLocation, const FVector& CapsuleDown, const float CapsuleRadius, const FVector& TestImpactPoint) const
{
...
void UPlanetGravityMovementComponent::ComputeFloorDist(const FVector& CapsuleLocation, float LineDistance, float SweepDistance, FFindFloorResult& OutFloorResult, float SweepRadius, const FHitResult* DownwardSweepResult) const
{
...
bool UPlanetGravityMovementComponent::FloorSweepTest(FHitResult& OutHit, const FVector& Start, const FVector& End, ECollisionChannel TraceChannel,
...
bool UPlanetGravityMovementComponent::IsValidLandingSpot(const FVector& CapsuleLocation, const FHitResult& Hit) const
{
	if (!Hit.bBlockingHit)
	{
		return false;
...
bool UPlanetGravityMovementComponent::ShouldCheckForValidLandingSpot(float DeltaTime, const FVector& Delta, const FHitResult& Hit) const
{
	const FVector CapsuleUp = GetComponentAxisZ();
...
bool UPlanetGravityMovementComponent::ShouldComputePerchResult(const FHitResult& InHit, bool bCheckRadius) const
{
...
bool UPlanetGravityMovementComponent::ComputePerchResult(const float TestRadius, const FHitResult& InHit, const float InMaxFloorDist, FFindFloorResult& OutPerchFloorResult) const
{
...
bool UPlanetGravityMovementComponent::StepUp(const FVector& GravDir, const FVector& Delta, const FHitResult& InHit, struct UCharacterMovementComponent::FStepDownResult* OutStepDownResult)
{
...
void UPlanetGravityMovementComponent::HandleImpact(const FHitResult& Impact, float TimeSlice, const FVector& MoveDelta)
{
	if (CharacterOwner)
	{
...
void UPlanetGravityMovementComponent::ApplyImpactPhysicsForces(const FHitResult& Impact, const FVector& ImpactAcceleration, const FVector& ImpactVelocity)
{
...
void UPlanetGravityMovementComponent::DisplayDebug(UCanvas* Canvas, const FDebugDisplayInfo& DebugDisplay, float& YL, float& YPos)
{
...
void UPlanetGravityMovementComponent::VisualizeMovement() const
{
...
FVector UPlanetGravityMovementComponent::ConstrainInputAcceleration(const FVector& InputAcceleration) const
{
...
void UPlanetGravityMovementComponent::ServerMoveHandleClientError(float ClientTimeStamp, float DeltaTime, const FVector& Accel, const FVector& RelativeClientLoc, UPrimitiveComponent* ClientMovementBase, FName ClientBaseBoneName, uint8 ClientMovementMode)
{
...
void UPlanetGravityMovementComponent::ClientAdjustPosition_Implementation(float TimeStamp, FVector NewLocation, FVector NewVelocity, UPrimitiveComponent* NewBase, FName NewBaseBoneName, bool bHasBase, bool bBaseRelativePosition, uint8 ServerMovementMode)
{
...
void UPlanetGravityMovementComponent::CapsuleTouched(UPrimitiveComponent* OverlappedComp, AActor* Other, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	if (!bEnablePhysicsInteraction)
	{
		return;
...
void UPlanetGravityMovementComponent::ApplyDownwardForce(float DeltaSeconds)
{
...
void UPlanetGravityMovementComponent::ApplyRepulsionForce(float DeltaSeconds)
{
...
void UPlanetGravityMovementComponent::ApplyAccumulatedForces(float DeltaSeconds)
{
...
FVector UPlanetGravityMovementComponent::GetGravity() const
{
...
FVector UPlanetGravityMovementComponent::GetGravityDirection(bool bAvoidZeroGravity) const
{
...
float UPlanetGravityMovementComponent::GetGravityMagnitude() const
{
	return FMath::Abs(GetGravityZ());
}

void UPlanetGravityMovementComponent::SetGravityDirection(const FVector& NewGravityDirection)
{
	SetCustomGravityDirection(NewGravityDirection.GetSafeNormal());
}

FORCEINLINE void UPlanetGravityMovementComponent::SetCustomGravityDirection(const FVector& NewCustomGravityDirection)
{
...
void UPlanetGravityMovementComponent::ClientSetCustomGravityDirection_Implementation(const FVector& NewCustomGravityDirection)
{
	SetCustomGravityDirection(NewCustomGravityDirection);
}

void UPlanetGravityMovementComponent::ClientClearCustomGravityDirection_Implementation()
{
	SetCustomGravityDirection(FVector::ZeroVector);
}

void UPlanetGravityMovementComponent::ClientSetGravityPoint_Implementation(const FVector& NewGravityPoint)
{
	GravityPoint = NewGravityPoint;
}

void UPlanetGravityMovementComponent::ClientClearGravityPoint_Implementation()
{
	GravityPoint = FVector::ZeroVector;
}

void UPlanetGravityMovementComponent::ClientSetGravityScale_Implementation(float NewGravityScale)
{
	GravityScale = NewGravityScale;
}

void UPlanetGravityMovementComponent::UpdateGravity(float DeltaTime)
{
...
FRotator UPlanetGravityMovementComponent::ConstrainComponentRotation(const FRotator& Rotation) const
{
...
FORCEINLINE FVector UPlanetGravityMovementComponent::GetComponentAxisX() const
{
...
FORCEINLINE FVector UPlanetGravityMovementComponent::GetComponentAxisZ() const
{
...
FVector UPlanetGravityMovementComponent::GetComponentDesiredAxisZ() const
{
...
void UPlanetGravityMovementComponent::UpdateComponentRotation()
{
	if (!HasValidData())
	{
		return;
